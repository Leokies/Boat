...
- [ASK](#ASK)
	...
	- [Establish metrics](#Establish_metrics)
- [Prepare](#Prepare)	
- [Process](#Process)
- [Analysis](#Analysis)
 	- [Python](#Python)
...
I am searching for a sailboat that is ocean capable, suitable for living aboard, and falls within my budget. As a student, my budget is limited to €30,000, so price will be the primary consideration. Based on my previous sailing experience, I am looking for a minimum boat length of 9 meters. This size will provide enough space for myself and my girlfriend to live comfortably aboard, even during long ocean passages.
...
Moving on, let's focus on the "conditions" column which contains non-uniform sentences indicating whether the boat is new or used. 

To standardize the form, we can use the CASE WHEN function to check for the occurrence of "new" or "used" within the cell, and output only the respective word.
...
The final step involves the "loc" column which contains inconsistent sentences but includes the name of the country we are interested in. 

To extract only the country name, we can utilize a combination of the LEFT, NULLIF, and CHARINDEX functions.
...
The matrix shows a moderate to strong correlation between `price` and `boat length` (0.67) as well as `price` and `boat width` (0.66). 

Furthermore, we can see that the `price` is only weakly related to the year of construction`built` (0.18). 

Besides, we can notice that there is a strong correlation between `boots length` and `boat width` (0.81).
...
### Using this valuable information, I began my search for the perfect sailboat in Italy. It was not an easy journey and took three months of diligent effort, but my persistence paid off when I finally stumbled upon my dream boat in Rome....
